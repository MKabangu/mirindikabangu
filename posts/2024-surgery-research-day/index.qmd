...
author: "Mirindi Kabangu & Noor Malik"
date: "2024-06-14"
image: "image.jpg"
...
Preliminary statistical analyses were conducted to assess demographic differences between the stented and non-stented groups among pediatric patients with Idiopathic Intracranial Hypertension. The results indicated that there were no significant differences in age between the two groups, as evidenced by a p-value of X.XX from the T-test. Similarly, the Wilcoxon rank-sum test showed no significant differences in BMI percentiles between the groups, with a p-value of Y.YY. However, chi-squared tests for categorical variables such as sex and puberty stage revealed significant differences. The chi-squared test for sex resulted in a p-value of Z.ZZ, suggesting a significant variation in the distribution of sexes between the two groups. Additionally, differences in puberty stages were statistically significant with a p-value of A.AA, indicating distinct patterns in puberty progression between the stented and non-stented groups.
...
ggplot(data, aes(x = stented_status, y = out_resolution, fill = stented_status)) +
  ...
  labs(title = "<span style='color:#990000'>Stenting</span> was associated with greater reductions in pressure gradient and symptom resolution, compared to <span style='color:#f2dfdf'>no stent</span>.") +
  theme_minimal() +
  ylim(0,1) +
...
The graph provides a visual comparison between the stented and non-stented groups, highlighting the effectiveness of stenting in alleviating the key symptoms of IIH. For instance, the resolution of headaches and vision changes, which are among the most debilitating symptoms of IIH, appears to be markedly better in patients who underwent stenting. This suggests that venous sinus stenting not only helps in normalizing intracranial pressure but also significantly improves patient outcomes in terms of symptom relief.
...
```{r}
ggplot(data, aes(x = bmi_percentile, y = out_resolution, color = factor(tx_stent))) +
...
```{r}
ggplot(data, aes(x = age, y = out_resolution, color = factor(tx_stent))) +
  geom_jitter(alpha = 0.5) +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE) +
  labs(title = "The influence of <span style='color:#990000'>age</span> on treatment outcomes was relatively minor") +
  theme_minimal() +
  theme(legend.position = "none",
        title = element_blank(),
        axis.text.y = element_blank(),
